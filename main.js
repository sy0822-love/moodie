...
function normalizeAnalysis(data) {
  ...
  for (const category of CATEGORIES) {
    ...
    if (Number.isFinite(value)) scores[category] = clamp(value, 1, 5);
  }
  const happy = unique((data?.happy || []).filter((x) => CATEGORIES.includes(x)));
  const unhappy = unique((data?.unhappy || []).filter((x) => CATEGORIES.includes(x)));
...
function updatePreview() {
  ...
  updatePreviewTone(analysis.avgScore, Boolean(content.trim()));
  renderTags(elements.previewTags, analysis);
  elements.previewAdvice.textContent = content.trim() ? analysis.recommendation : "寫下一點內容後，這裡會先用本地模型預判分類。";
}
...
function bindEvents() {
  ...
  elements.rangeSelect.addEventListener("change", renderInsights);
  ...
  elements.syncKeyInput.addEventListener("change", () => {
    const syncKey = getSyncKey();
    if (syncKey) {
      localStorage.setItem(SYNC_KEY_STORAGE, syncKey);
